...
That is, we can swap out =grid= for any other function and see space
distorted in the same way; the grid was just used there to make it
more obvious how space is distorted.
...
void main()
{
    ...
    vec2 uv2 = sine_warp(uv - m, ampl, freq_r, -time*freq_t);
    vec2 polar = rect2polar(uv2);
    
    gl_FragColor = vec4(hsv2rgb(vec3(polar.y / PI / 2.0, 1.0, 1.0)), 1.0);
}
#+END_SRC
